...
function initConfig(path = "dhal.json") {
  const resolved = (0, import_node_path4.resolve)(process.cwd(), path);
  if ((0, import_node_fs4.existsSync)(resolved)) {
    console.error(`Refusing to overwrite existing config: ${resolved}`);
    process.exitCode = 1;
    return;
  }
  (0, import_node_fs4.writeFileSync)(resolved, `${JSON.stringify(defaultConfig, null, 2)}
`);
...
function exportSchema(path) {
  ...
  if (path) {
    const resolved = (0, import_node_path4.resolve)(process.cwd(), path);
    (0, import_node_fs4.writeFileSync)(resolved, `${schema}
`);
    console.log(`Created ${resolved}`);
    return;
  }
...
`;
  if (outputPath) {
    const resolved = (0, import_node_path4.resolve)(process.cwd(), outputPath);
    (0, import_node_fs4.writeFileSync)(resolved, serialized);
...
async function replay(path, configPath = "dhal.json", json = false, failOnBlock = false) {
  ...
  const raw = (0, import_node_fs4.readFileSync)((0, import_node_path4.resolve)(process.cwd(), path), "utf8");
