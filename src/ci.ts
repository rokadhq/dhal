...
export type DhalCiFinding = {
  level: "error" | "warning";
  code: string;
  message: string;
};

export type DhalCiResult = {
  ok: boolean;
  findings: DhalCiFinding[];
};

export function evaluateDhalCiPolicy(config: DhalConfig): DhalCiResult {
  const findings: DhalCiFinding[] = [];
...
function isRuleEnabled(ruleName: string, config: DhalConfig): boolean {
  switch (ruleName) {
